...
BackpropSnapshot::BackpropSnapshot(
  ...
  : mUseFDOverride(world->getUseFDOverride()),
    ...
    mNumBouncing(0)
{
  mIntegrationBackups = world->integrationBackups;
  mHasBacktrack = world->getBacktrack();
...
void BackpropSnapshot::setGradientMatrices(
  std::vector<std::shared_ptr<ConstrainedGroupGradientMatrices>>& newMatrices)
{
  mNumConstraintDim = 0;
  mNumClamping = 0;
  mNumUpperBound = 0;
  mNumBouncing = 0;
  mGradientMatrices.clear();
  for (auto gradientMatrix : newMatrices)
  {
    ...
    mNumUpperBound
      += gradientMatrix->getUpperBoundConstraintMatrix().cols();
...
void BackpropSnapshot::setWorld(WorldPtr world, bool afterToi)
{
  int idx = afterToi ? 1 : 0;

  mPreStepPosition = mIntegrationBackups[idx].initPosition;
  mPreStepVelocity = mIntegrationBackups[idx].initVelocity;
  mPreStepTorques = mIntegrationBackups[idx].torque;

  mPreConstraintVelocities = mIntegrationBackups[idx].midVelocity;
  mPreStepLCPCache = mIntegrationBackups[idx].preLCPSolution;

  mPostStepPosition = mIntegrationBackups[idx].nextPosition;
  mPostStepVelocity = mIntegrationBackups[idx].nextVelocity;
  mPostStepTorques = mIntegrationBackups[idx].torque;

  mAcceleration = mIntegrationBackups[idx].acceleration;

  mTimeStep = mIntegrationBackups[idx].timestep;
  setGradientMatrices(mIntegrationBackups[idx].gradientMatrices);
  ...
  world->setTimeStep(mTimeStep);
}

const Eigen::MatrixXs BackpropSnapshot::getPosTimeJacobian(
    simulation::WorldPtr world)
{
  ...
  Eigen::VectorXs J, J_c, v;
  double v_n;
  J = Eigen::VectorXs::Zero(rows);
  ...
  Eigen::MatrixXs result = J / cols;
  return result;
}

void BackpropSnapshot::saveToiGradients(simulation::WorldPtr world)
{
  Eigen::MatrixXs A_c = getClampingConstraintMatrix(world);
  int rows = A_c.rows();
  int cols = A_c.cols();

  std::vector<double> toiGradients_list;
  Eigen::VectorXd toiGradients;

  Eigen::VectorXs J_c, v;
  double v_n;

  for (int c = 0; c < cols; c++) {
    J_c = A_c.col(cols - 1);
    v = world->getVelocities();
    v_n = - J_c.dot(v);
    toiGradients_list.push_back(1 / v_n);
  }

  toiGradients = Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>(toiGradients_list.data(), toiGradients_list.size());
  std::cout<<toiGradients<<std::endl;

  world->setToiGradients(toiGradients);
}
    ...
    bool exploreAlternateStrategies)
{
  ...
  if (exploreAlternateStrategies == true) {
    std::cerr<<"ExploreAlternateStrategies setting wrong"<<std::endl;
    return;
  }
#endif
  ...
  if (mHasBacktrack) {
    logging::debug("[backprop] Have backtrack. Setting world from [t+TOI, t+dt).");
    setWorld(world, true);
    ...
    LossGradient toiTimestepLoss;
    toiTimestepLoss.lossWrtPosition = Eigen::VectorXs::Zero(mNumDOFs);
    toiTimestepLoss.lossWrtVelocity = Eigen::VectorXs::Zero(mNumDOFs);
    toiTimestepLoss.lossWrtTorque = Eigen::VectorXs::Zero(mNumDOFs);
    toiTimestepLoss.lossWrtMass = Eigen::VectorXs::Zero(world->getMassDims());

    const Eigen::MatrixXs& toiPosPos = getPosPosJacobian(world, thisLog);
    const Eigen::MatrixXs& toiPosVel = getPosVelJacobian(world, thisLog);
    const Eigen::MatrixXs& toiVelPos = getVelPosJacobian(world, thisLog);
    const Eigen::MatrixXs& toiVelVel = getVelVelJacobian(world, thisLog);
    const Eigen::MatrixXs& toiForceVel = getControlForceVelJacobian(world, thisLog);
    const Eigen::MatrixXs& toiMassVel = getMassVelJacobian(world, thisLog);
    Eigen::VectorXs toiAcceleration = mAcceleration;

    toiTimestepLoss.lossWrtPosition
      = toiPosPos.transpose() * nextTimestepLoss.lossWrtPosition
      + toiPosVel.transpose() * nextTimestepLoss.lossWrtVelocity;
    toiTimestepLoss.lossWrtVelocity
      = toiVelPos.transpose() * nextTimestepLoss.lossWrtPosition
      + toiVelVel.transpose() * nextTimestepLoss.lossWrtVelocity;
    toiTimestepLoss.lossWrtTorque
      = toiForceVel.transpose() * nextTimestepLoss.lossWrtVelocity;
    toiTimestepLoss.lossWrtMass
      = toiMassVel.transpose() * nextTimestepLoss.lossWrtVelocity;
    ...
    // const Eigen::MatrixXs& posToi = getPosTimeJacobian(world);
    // const Eigen::MatrixXs& velToi = mTimeStep * getPosTimeJacobian(world);
    // saveToiGradients(world);
    ...
    logging::debug("[backprop] toiAcceleration:\n{0}", toiAcceleration);
    logging::debug("[backprop] preAcceleration:\n{0}", preAcceleration);
    ...
    logging::debug("[backprop] posPos:\n{0}", posPos);
    logging::debug("[backprop] posVel:\n{0}", posVel);
    logging::debug("[backprop] velPos:\n{0}", velPos);
    logging::debug("[backprop] velVel:\n{0}", velVel);
    logging::debug("[backprop] posToi:\n{0}", posToi);
    logging::debug("[backprop] velToi:\n{0}", velToi);
    logging::debug("[backprop] posToiPos:\n{0}", posToiPos);
    logging::debug("[backprop] posToiVel:\n{0}", posToiVel);
    logging::debug("[backprop] velToiPos:\n{0}", velToiPos);
    logging::debug("[backprop] velToiVel:\n{0}", velToiVel);

    thisTimestepLoss.lossWrtPosition
      = posPos.transpose() * toiTimestepLoss.lossWrtPosition
      + posVel.transpose() * toiTimestepLoss.lossWrtVelocity
      + posToiPos * nextTimestepLoss.lossWrtPosition
      + posToiVel * nextTimestepLoss.lossWrtVelocity;
    thisTimestepLoss.lossWrtVelocity
      = velPos.transpose() * toiTimestepLoss.lossWrtPosition
      + velVel.transpose() * toiTimestepLoss.lossWrtVelocity
