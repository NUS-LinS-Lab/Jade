...
public:
  ...
  void setWorld(simulation::WorldPtr world, bool afterToi);

  void setGradientMatrices(
    std::vector<std::shared_ptr<ConstrainedGroupGradientMatrices>>& newMatrices);

    const Eigen::MatrixXs getPosTimeJacobian(
      simulation::WorldPtr world);

    void saveToiGradients(simulation::WorldPtr world);
  ...
  std::vector<std::shared_ptr<ConstrainedGroupGradientMatrices>>
      mGradientMatrices;  // As temporary value for calculation
  std::vector<std::shared_ptr<ConstrainedGroupGradientMatrices>>
      mPreStepGradientMatrices;  // [t, t + TOI)
  std::vector<std::shared_ptr<ConstrainedGroupGradientMatrices>>
      mToiGradientMatrices;  // [t + TOI, t + dt)
...
protected:
  ...
  /// The acceleration during an integration
  Eigen::VectorXs mAcceleration;

  /// If a contact event happens during this snapshot of [t, t + dt)
  bool mHasBacktrack;
  std::vector<simulation::IntegrationBackupData> mIntegrationBackups;
